...
async function generateOrderImage(input) {
  ...
  try {
    ...
    console.error('Failed to load logo from:', logoPath, err)
  }
  try {
  ...
  const textLeft = logo ? margin + logoSize + 20 : margin

  ctx.fillStyle = colors.title
  ctx.font = 'bold 24px sans-serif'
  ctx.textBaseline = 'top'
  ctx.textAlign = 'left'
  const titleY = logo ? margin + (logoSize - 24) / 2 : margin + 2
  ctx.fillText(title, textLeft, titleY)

  ctx.strokeStyle = theme === 'dark' ? '#303030' : '#EEE7D9'
  ctx.lineWidth = 1
  ctx.beginPath()
  ctx.moveTo(margin, margin + Math.max(logoSize, titleHeight))
  ctx.lineTo(width - margin, margin + Math.max(logoSize, titleHeight))
  ctx.stroke()
  ...
  for (const g of groups) {
    ...
    for (const it of g.items) {
      ...
      if (it.note && printedLines < maxLines) {
        ...
        ctx.fillText(`备注：${it.note}`, margin + 28, y + 9)
        y += 18
        printedLines++
      }
    }
  ...
  ctx.font = '12px sans-serif'
  ctx.fillText(`EatWhat ~ ${nowText()}`, width - margin, height - margin - 8)
  ctx.restore()
